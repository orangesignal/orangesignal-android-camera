...
/**
 * Android 4.0 (API レベル 14) 以降向けの OpenGL ES 2.0 を使用するプレビューの実装を提供します。
 * 
 * @author 杉澤 浩二
 * @deprecated 未検証
 */
...
public class GLES20PreviewICS extends GLTextureView implements CameraView.Preview, Camera.PictureCallback {
	 ...
	 */
	public GLES20PreviewICS(final Context context) {
		super(context);
		initialize(context);
	}
	 ...
	 */
	public GLES20PreviewICS(final Context context, final AttributeSet attrs) {
		super(context, attrs);
		initialize(context);
	}
	 ...
	 * @param defStyle 
	 */
	public GLES20PreviewICS(final Context context, final AttributeSet attrs, final int defStyle) {
	...
	public boolean onSurfaceTextureDestroyed(final SurfaceTexture surface) {
		// サーフェスが破棄されるタイミングで最大テクスチャサイズを 0 クリアすることによって、
		// サーフェスが初期化されていない状態で、startPreview(int, int, CameraView.CameraStateListener) 経由で prepareStartPreview() が実行されないようにします。
		mMaxSize = 0;
	...
	public void startPreview(final int measurePreviewWidth, final int measurePreviewHeight, final CameraView.CameraStateListener listener) {
		synchronized (this) {
			...
			if (mMaxSize != 0) {
				...
				mRequiredStartPreview = true;
			}
		}
	}

	void setMaxSize(final int maxTextureSize) {
		mMaxSize = maxTextureSize;

		if (mRequiredStartPreview) {
			mRequiredStartPreview = false;
			prepareStartPreview();
		}
	}
	...
	private void prepareStartPreview() {
		synchronized (this) {
			...
			if (mMeasurePreviewWidth > 0 && mMeasurePreviewHeight > 0) {
				...
				mCameraHelper.setupOptimalPreviewSizeAndPictureSize(mMeasurePreviewWidth, mMeasurePreviewHeight, mMaxSize);
			}
...
//				mCameraHelper.cancelAutoFocus();
	...
	@TargetApi(Build.VERSION_CODES.ICE_CREAM_SANDWICH)
	private final class Renderer extends GLES20FramebufferObjectRenderer implements SurfaceTexture.OnFrameAvailableListener {
		...
		public void prepareStartPreview() {
			...
			try {
				mCameraHelper.setPreviewTexture(mPreviewTexture);
		...
		public void onSurfaceCreated(final EGLConfig config) {
			...
			GLES20Utils.setupSampler(GL_TEXTURE_EXTERNAL_OES, GL_LINEAR, GL_NEAREST);
			// 現在のテクスチャへの紐付けを解除します。
			 ...
			 */
			mPreviewTexture = new SurfaceTexture(mTexName);
			mPreviewTexture.setOnFrameAvailableListener(this);

			mFramebufferObject = new GLES20FramebufferObject();
			mPreviewShader = new GLES20PreviewShader(GL_TEXTURE_EXTERNAL_OES);
		...
		@Override
		public synchronized void onFrameAvailable(final SurfaceTexture surface) {
			/*
