 ...
 * 
 * @version android-17
 * @see http://stackoverflow.com/questions/12662164/glsurfaceview-onresume-tabbed-apllication-create-context
 */
...
public class GLSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
	...
	private void initializeGlEsVersion(final Context context) {
		final ActivityManager am = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
		final ConfigurationInfo info = am.getDeviceConfigurationInfo();
		sGLESVersion = info.reqGlEsVersion;
	}

	private static final int DONUT = 4;

	private void init(final Context context) {
		if (Integer.parseInt(Build.VERSION.SDK) >= DONUT && sGLESVersion == 0) {
	...
	private static class EglHelper {
		public EglHelper(final WeakReference<GLSurfaceView> glSurfaceViewWeakRef) {
		...
		GL createGL() {
			...
			final GLSurfaceView view = mGLSurfaceViewWeakRef.get();
			if (view != null) {
				...
				if ((view.mDebugFlags & (DEBUG_CHECK_GL_ERROR | DEBUG_LOG_GL_CALLS)) != 0) {
					...
					if ((view.mDebugFlags & DEBUG_LOG_GL_CALLS) != 0) {
						log = new LogWriter(GLSurfaceView.TAG);
					}
	...
	static class GLThread extends Thread {
		GLThread(final WeakReference<GLSurfaceView> glSurfaceViewWeakRef) {
			super();
			mWidth = 0;
			mHeight = 0;
			mRequestRender = true;
